...
export default function ViewProfilePage() {
  ...
  const tabs = ['Overview', 'Experience', 'Education', 'Connections', 'Activities'];
  ...
  if (!profile) return (
    <div className="max-w-5xl mx-auto text-center py-12">
      <p className="text-gray-500">Profile not found. <Link href="/alumni" className="text-[#001145] font-medium hover:underline">Browse Alumni</Link></p>
    </div>
  );
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 items-center text-[#001145]">
                    <span className="font-semibold">{profile.degree || 'Degree'}</span>
                    <span className="w-1 h-1 rounded-full bg-[#001145]/40"></span>
                    <span className="font-medium">{profile.major || 'Major'}</span>
                    <span className="w-1 h-1 rounded-full bg-[#001145]/40"></span>
                  ...
                  <div className="mt-4 p-4 bg-white/60 backdrop-blur-sm rounded-xl border border-white/50 max-w-2xl">
                    ...
                    {profile.isVerified && <div className="mt-2 inline-flex items-center gap-1 bg-white text-[#001145] px-2 py-1 rounded text-xs font-bold shadow-sm"><Check size={12} strokeWidth={3} /> Verified Alumni</div>}
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  {connectionStatus === 'connected' ? (
                    <Button variant="secondary" leftIcon={<Check size={16} />} className="shadow-sm">Connected</Button>
                  ) : connectionStatus === 'pending' ? (
                    <Button variant="outline" leftIcon={<Clock size={16} />} className="bg-white border-white/50">Pending</Button>
                  ) : (
                    <Button onClick={handleConnect} leftIcon={<UserPlus size={16} />} className="shadow-lg hover:shadow-xl transition-shadow bg-[#001145] hover:bg-[#001145]/90">Connect</Button>
                  )}
                  <Link href={`/messages?user=${id}`}>
                    <Button variant="outline" leftIcon={<MessageCircle size={16} />} className="w-full bg-white border-white/50 hover:bg-white">Message</Button>
                  </Link>
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl p-8 border border-[#e4f0ff] min-h-[500px]">
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-[#e4f0ff]/50 p-6 rounded-2xl border border-[#e4f0ff]">
                    ...
                    <p className="mt-2 text-[#001145] font-medium">{profile.degree || 'Degree'} in {profile.major || 'Major'}</p>
            ...
            {activeTab === 'Connections' && (
              <div className="flex flex-col items-center justify-center h-64 text-center">
                ...
                <h3 className="text-2xl font-bold text-[#001145]">Private Network</h3>
                <p className="text-[#001145]/60 mt-2">Connect with this user to see their connections.</p>
              </div>
            )}
