    ...
    <div className="w-full h-[calc(100vh-100px)] flex gap-6 max-w-7xl mx-auto">
      ...
      <div className={`hidden lg:flex flex-col w-80 bg-white rounded-3xl border border-[#e4f0ff] p-6 shadow-sm flex-shrink-0 transition-all duration-300`}>
        ...
        <div className="space-y-6">
          <div>
            <h3 className="text-xs font-bold text-[#001145]/40 uppercase tracking-wider mb-3">Capabilities</h3>
          ...
          <div>
            ...
            <div className="space-y-2">
              {suggestions.map((item, idx) => (
                <button key={idx} className="w-full flex items-center gap-3 p-3 bg-white border border-[#e4f0ff] rounded-xl text-sm font-medium text-[#001145] hover:bg-[#e4f0ff] transition-colors text-left group shadow-sm hover:shadow-md">
                  <span className="text-[#001145]/60 group-hover:text-[#001145] transition-colors">{item.icon}</span>
                  <span>{item.text}</span>
                </button>
              ))}
            </div>
